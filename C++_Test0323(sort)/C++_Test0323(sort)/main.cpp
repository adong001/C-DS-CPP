...
{
private:
	static void GetPivot(vector<int>& arr,int start, int end)//随机获取基准元素，优化算法
	{
		srand((unsigned int)(time(NULL)));
		int pos = rand() % (end - start + 1) + start;
		Swap(arr[pos],arr[start]);//让start成为基准元素
	}
public:
	static int Deal_Quick_Sort(vector<int>& arr, int start, int end, bool(*cmp)(T, T,bool) = Less)//递归版快排
	{
	...
	static void Recursion_Quick_Sort(vector<int>& arr, int start, int end, bool(*cmp)(T, T, bool) = Less)//递归版快排
	{
		if (start < end)
		{
			int mid = Deal_Quick_Sort(arr, start, end, cmp);//两边排序，左小于中间，右大于中间
			Quick_Sort(arr, start,mid - 1, cmp);//左右再递归排序
			Quick_Sort(arr, mid + 1,end, cmp);
		}
	}
	static void NonRecursion_Quick_Sort(vector<int>& arr, int start, int end, bool(*cmp)(T, T, bool) = Less)//非递归版
	{
		int left, right,mid;
		stack<T> st;
		st.push(end);
		...
		while (!st.empty())
		{
			...
			if (left < right)//防止越界
			{
				...
				//先将mid作为基准元素排完两边，左小于mid，右大于mid

				if (left < mid - 1)//不越界就将左边压入栈
				{
					st.push(mid - 1);
					st.push(left);
				}
				if (mid + 1 < right)//不越界就将右边压入栈
				{
					st.push(right);
					st.push(mid + 1);
				}
...
//	}
//public:
//	static int Deal_Quick_Sort(vector<int>& arr, int start, int end, bool(*cmp)(T, T, bool) = Less)//处理每一趟
//	{
//		int left = start, right = end;
